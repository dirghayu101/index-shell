...
export const GenerateSnippet = async (command, maxRetries = 3, delay = 1) => {
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      console.error(`Error calling Gemini (attempt ${retryCount + 1}/${maxRetries}): ${error.message}`);
      retryCount++;
      await new Promise(resolve => setTimeout(resolve, delay * 1000));
    }
  }
  console.error("Failed to call Gemini after all retries.");
  return null;
  }


async function callGemini(inputCommand) {
  ...
  const retrievedResult = JSON.parse(responseString);
